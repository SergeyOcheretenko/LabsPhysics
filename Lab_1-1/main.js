...
function random(min, max) {
  ...
  const s = (array, number) => {
    ...
    return Math.round(Math.sqrt(arrayDTpow.reduce((acc, elem) => acc + elem, 0) / number / (number - 1)) * 10000000) / 10000000;
  }
  ...
  const tabl = (array, number) => {
    ...
    let p = document.createElement('p');
      p.innerHTML = `Сумма значений периодов ( Σ(T) ) для ${number} замеров: ${Math.round((tablT(array).reduce((acc, elem) => acc + Number(elem), 0)) * 10000) / 10000} <br>Среднее значение периода ❬T❭ для ${number} замеров: ${average(array, number)} <br>Сумма квадратов абсолютных погрешностей ( Σ(△ T)² ) <br> для ${number} замеров: ${Math.round(arrayDTpow.reduce((acc, elem) => acc + elem, 0) * 100000000) / 100000000} <br>Среднее значение квадрата абсолютной погрешности <br>❬△ T)²❭ для ${number} замеров: ${Math.round((arrayDTpow.reduce((acc, elem) => acc + elem, 0)) / number * 1000000000) / 1000000000} <br>Случайная погрешность S для ${number} замеров: ${s(array, number)} <br>Систематическая погрешность σ: ${Number((0.01 / (5 * Math.sqrt(12))).toFixed(6))}`;
    //Таблыця
